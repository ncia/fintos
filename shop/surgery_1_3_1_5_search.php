...
<div class="surgery-search-container container m-t-30 m-b-50">
    <div class="main-heading m-b-30">
        <h2 class="f-s-24 fw-600"><i class="fas fa-search-plus text-success m-r-10"></i><strong>종 수술명</strong> <span class="text-muted">검색</span></h2>
    </div>
            ...
            <div class="row g-3 align-items-end">
                <div class="col-lg-2 col-md-6">
                    ...
                    <select id="surgeryMode" class="form-select border-gray-200 py-2 rounded-3">
                        ...
                        <option value="1-8">1-8종 수술</option>
                    </select>
                </div>
                <div class="col-lg-4 col-md-6">
                    ...
                    <input type="text" id="surgeryName" class="form-control border-gray-200 py-2 rounded-3" placeholder="예: 백내장, 암">
                </div>
                <div class="col-lg-2 col-md-6">
                    <label class="form-label f-s-15 fw-bold text-dark w-100 text-center"><i class="fas fa-list m-r-5 text-secondary"></i>1-3종 수술</label>
                    <select id="type3Select" class="form-select border-gray-200 py-2 rounded-3">
                ...
                <div class="col-lg-2 col-md-6">
                    <label class="form-label f-s-15 fw-bold text-dark w-100 text-center"><i class="fas fa-sort-numeric-up m-r-5 text-secondary"></i>1-5종 수술</label>
                    <select id="type5Select" class="form-select border-gray-200 py-2 rounded-3">
                        <option value="">전체</option>
                        <option value="1종">1종</option>
                        <option value="2종">2종</option>
                        <option value="3종">3종</option>
        ...
        <div id="surgeryResultArea" class="px-2 px-md-4 pb-4">
            <div class="text-center py-5">
                <div class="p-5">
                    ...
                    <h5 class="text-dark fw-bold mb-2">종 수술 정보를 검색해 보세요!</h5>
                    <p class="text-secondary f-s-14 mb-0">수술명 또는 종별 구분을 입력하여 상세 정보를 확인할 수 있습니다.</p>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    function searchSurgery(page = 1) {
        ...
        const type_3 = $('#type3Select').val();
        const type_5 = $('#type5Select').val();
        const mode = $('#surgeryMode').val();
        ...
        $.ajax({
            url: '<?php echo G5_SHOP_URL; ?>/surgery_1_3_1_5_search_api.php',
            type: 'GET',
            data: {
                q: q,
                type_3: type_3,
    ...
    function renderResults(data, page) {
        ...
        html += '<thead class="bg-gray-50"><tr class="text-center"><th>수술명</th><th>1-3종 구분</th><th>1-5종 구분</th></tr></thead>';
        html += '<tbody class="f-s-16">';
        
        items.forEach(function(item) {
            html += '<tr>';
            html += '<td class="px-4 fw-600">' + item.name + '</td>';
            html += '<td class="text-center"><span class="badge ' + getBadgeClass(item.type_1_3) + '">' + item.type_1_3 + '</span></td>';
            html += '<td class="text-center"><span class="badge ' + getBadgeClass(item.type_1_5) + '">' + item.type_1_5 + '</span></td>';
            html += '</tr>';
        });
    ...
    function getBadgeClass(type) {
        if (type.indexOf('1종') > -1) return 'badge-type-1';
        if (type.indexOf('2종') > -1) return 'badge-type-2';
        if (type.indexOf('3종') > -1) return 'badge-type-3';
        if (type.indexOf('4종') > -1) return 'badge-type-4';
        if (type.indexOf('5종') > -1) return 'badge-type-5';
    ...
    $('#surgeryName').on('keypress', function(e) { if (e.which === 13) searchSurgery(1); });
    ...
    $('#surgeryMode').on('change', function() {
        if ($(this).val() === '1-7') {
            ...
            window.location.href = '<?php echo G5_SHOP_URL; ?>/surgery_1_8_search.php';
        } else {
            searchSurgery(1);
        }
    });
