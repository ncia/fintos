...
<div class="mdfee-info-container container m-t-30 m-b-50">
    <div class="main-heading m-b-30">
        <h2 class="f-s-24 fw-600"><i class="fas fa-stethoscope text-primary m-r-10"></i><strong>진료 수가 정보</strong> <span class="text-muted">조회</span></h2>
    </div>
            ...
            <div class="row g-3 align-items-end">
                <div class="col-lg-2 col-md-6">
                    ...
                    <select id="mdfeeTypeChoice" class="form-select border-gray-200 py-2 rounded-3">
                        ...
                        <option value="diagnosis" selected>진료 수가</option>
                    </select>
                </div>
                <div class="col-lg-3 col-md-6">
                    ...
                    <input type="text" id="korNm" class="form-control border-gray-200 py-2 rounded-3" placeholder="예: 스텐트삽입술, 진찰료">
                </div>
                <div class="col-lg-2 col-md-6">
                    ...
                    <input type="text" id="mdfeeCd" class="form-control border-gray-200 py-2 rounded-3" placeholder="예: M6561">
                </div>
                <div class="col-lg-3 col-md-8">
                    ...
                    <input type="text" id="mdfeeDivNo" class="form-control border-gray-200 py-2 rounded-3" placeholder="예: 자656가">
                </div>
                <div class="col-lg-2 col-md-4">
                    <button type="button" id="mdfeeSearchBtn" class="btn btn-primary w-100 py-2 rounded-3 shadow-sm hover-up f-s-15 fw-bold transition text-white">조회하기</button>
                </div>
            </div>
        </div>
        ...
        <div id="mdfeeResultArea" class="px-2 px-md-4 pb-4">
            <div class="text-center py-5">
                <div class="p-5">
                    ...
                    <h5 class="text-dark fw-bold mb-2">전국 진료 수가 정보를 검색해 보세요!</h5>
    ...
    <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-primary text-white border-0 py-3">
                <h5 class="modal-title fw-600"><i class="fas fa-stethoscope m-r-10"></i>수가 상세 정보</h5>
                    ...
                    <table class="table detail-modal-table mb-0">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td id="modal_korNm" class="fw-bold text-primary border-bottom-0">-</td>
                            </tr>
                            <tr>
...
$(document).ready(function() {
    ...
    function searchMdfee(page = 1) {
        ...
        $('#mdfeeResultArea').html('<div class="text-center py-5 my-5"><div class="spinner-border text-primary" role="status" style="width: 3rem; height: 3rem;"><span class="visually-hidden">Loading...</span></div><p class="m-t-20 text-muted f-s-15">데이터를 조회 중입니다...</p></div>');

        $.ajax({
            url: '<?php echo G5_SHOP_URL; ?>/getDiagnossMdfeeList_api.php',
            type: 'GET',
            data: {
                korNm: korNm,
                mdfeeCd: mdfeeCd,
    ...
    function renderResults(data, page) {
        ...
        items.forEach(function(item) {
            ...
            html += '<tr>';
            html += '<td class="text-center font-monospace fw-600 text-primary">' + displayCode + '</td>';
            html += '<td class="text-center">' + displayDiv + '</td>';
            html += '<td class="text-start px-4">' + displayName + '</td>';
            html += `<td class="text-center"><button class="btn btn-xs btn-outline-primary rounded-pill detail-btn" data-item='${itemData}'>보기</button></td>`;
            html += '</tr>';
        });
...
.mdfee-table tbody tr:hover { background-color: #f0f7ff; }
...
.pagination-modern .page-item.active .page-link { background-color: #0d6efd; color: #fff; border-color: #0d6efd; }
