    ...
    public function get_category_select($arr) {
        if(is_array($arr)) {
            foreach ($arr as $k => $info) {
                ...
                // 전체 카테고리 경로 표시
                $category_path = function_exists('get_shop_category_path') ? get_shop_category_path($info['ca_id']) : $info['ca_name'];
                $output['select'] .= "<option value=\"{$info['ca_id']}\">$nbsp{$category_path}</option>\n";
