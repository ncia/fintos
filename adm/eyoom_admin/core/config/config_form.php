 ...
 * 관리자 접근통제 설정
 */
if (!isset($config['cf_use_admlogin'])) {
    sql_query("ALTER TABLE `{$g5['config_table']}`
                ADD `cf_use_admlogin` TINYINT(4) NOT NULL DEFAULT '0' AFTER `cf_eyoom_admin_theme`,
                ADD `cf_admlogin_ip` TEXT NOT NULL AFTER `cf_use_admlogin` ", true);
}
