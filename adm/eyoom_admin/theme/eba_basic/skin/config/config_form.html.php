    ...
    <div class="tab-content">
            ...
            <div class="adm-form-table m-b-20">
                ...
                <div class="adm-form-tr-wrap">
                    ...
                    <div class="adm-form-tr tr-r">
                        <div class="adm-form-td td-l">
                            <label for="cf_use_admlogin" class="label">관리자 접근통제 기능</label>
                        </div>
                        <div class="adm-form-td td-r">
                            <label class="checkbox">
                                <input type="checkbox" name="cf_use_admlogin" value="1" id="cf_use_admlogin" <?php echo $config['cf_use_admlogin']?'checked':''; ?>><i></i> 사용
                            </label>
                            <div class="note"><strong>Note:</strong> 사용에 체크하시면 아래 지정한 아이피에서만 접근이 가능합니다.</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="adm-form-tr tr-l">
                        ...
                        <div class="adm-form-td td-r">
                            ...
                            <div class="note">
                                ...
                                <br><br>외부메일설정이나 기타 설정을 하지 않았다면, 도메인과 다른 헤더로 여겨 스팸이나 차단될 가능성이 있습니다.<br>name<?php echo domain_mail_host(); ?>과 같은 도메인 형식으로 설정할것을 권장합니다. <br><br>
                                <?php } ?>
                                <strong>Note:</strong> 관리자 접근통제 기능 사용시 관리자 로그인이 제한된 환경에 있을 경우, 관리자 메일로 접근가능한 인증키를 발송합니다.<br>
                                <strong>Note:</strong> 환경설정 > 기본환경설정 > 메일환경설정 > SMTP 인증메일 보내기 설정을 완료하신 후, 관리자 접근통제 기능을 사용하시기를 권장합니다.
                            </div>
                        </div>
                    </div>
                    <div class="adm-form-tr tr-r">
                        <div class="adm-form-td td-l">
                            <label for="cf_admlogin_ip" class="label">관리자 접근 가능 아이피<br><a href="javascript:void(0);" id="btn_apply_my_ip" class="btn-e btn-e-md btn-e-gray">MY IP 적용하기 <i class="fas fa-arrow-right"></i></a></label>
                        </div>
                        <div class="adm-form-td td-r">
                            <label class="textarea">
                                <textarea name="cf_admlogin_ip" id="cf_admlogin_ip" rows="8"><?php echo get_sanitize_input($config['cf_admlogin_ip']); ?></textarea>
                            </label>
                            <div class="note"><strong>Note:</strong> 입력된 IP의 컴퓨터만 관리자로 로그인하실 수 있습니다. (엔터로 구분)</div>
                        </div>
                    </div>
                </div>
                <div class="adm-form-tr">
        ...
        <div class="tab-pane" id="anc_cf_join" role="tabpanel" aria-labelledby="anc_cf_join_tab">
            <div class="adm-form-table m-b-20">
                ...
                <div class="adm-form-tr adm-sm-100">
                    <div class="adm-form-td td-l">
                        <label for="cf_use_promotion" class="label">회원가입 약관 동의에<br>광고성 정보 수신 동의 표시 여부</label>
                    </div>
                    <div class="adm-form-td td-r">
                        <label for="cf_use_promotion" class="checkbox">
                            <input type="checkbox" name="cf_use_promotion" value="1" id="cf_use_promotion" <?php echo $config['cf_use_promotion'] ? 'checked' : ''; ?>><i></i> 사용
                        </label>
                        <div class="note"><strong>Note:</strong> <b>광고성 정보 수신 · 마케팅 목적의 개인정보 수집 및 이용 · 개인정보 제 3자 제공</b> 여부를 설정합니다. <b>SMS 또는 카카오톡</b> 사용 시 <b>개인정보 제3자 제공</b>이 활성화됩니다.</div>
                        <div class="note"><strong>Note:</strong> 동의한 회원에게 <b>카카오톡(친구톡)·문자</b>로 광고성 메시지를 발송할 수 있습니다.</div>
                        <div class="note"><strong>Note:</strong> <b>휴대전화번호</b> 사용을 위해서는 <b>기본환경설정 > 회원가입 > 휴대전화번호 입력</b>을 <b>[보이기]</b> 또는 <b>[필수입력]</b>으로 설정해야 하며, 미설정 시 수집이 불가합니다.</div>
                        <div class="note"><strong>Note:</strong> * 「정보통신망이용촉진및정보보호등에관한법률」에 따라 <b>광고성 정보 수신 동의</b>를 매 2년마다 반드시 확인해야 합니다.</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="tab-pane" id="anc_cf_sms" role="tabpanel" aria-labelledby="anc_cf_sms_tab">
            <div class="adm-form-table m-b-20">
                ...
                <div class="adm-form-tr icode_old_version">
                    ...
                    <div class="adm-form-td td-r">
                        <?php echo number_format($userinfo['coin'] ? $userinfo['coin'] : 0); ?> 원.
...
$(function() {
    ...
    // 관리자 접근 통제 IP 추가 함수
    function add_admlogin_ip(textarea, my_ip) {
        var current_ip = textarea.val().trim();
        var ips = current_ip ? current_ip.split('\n').map(function(ip) { return ip.trim(); }) : [];
        
        // 현재 IP 중복 확인
        var ipExists = ips.some(function(ip) { return ip === my_ip && ip !== ''; });
        
        if (!ipExists && my_ip) {
            textarea.val(current_ip ? current_ip + '\n' + my_ip : my_ip);
        } else if (ipExists) {
            alert('이미 추가된 IP입니다.');
        }
    }
    ...
    // MY IP 적용하기 버튼 클릭
    $("#btn_apply_my_ip").on("click", function(e) {
        e.preventDefault();
        add_admlogin_ip($("#cf_admlogin_ip"), "<?php echo $_SERVER['REMOTE_ADDR']; ?>");
    });
});
